...
def step_base(o, a, label):
    ...
    # v = np.dot(np.linalg.inv(np.diag(np.diag(G))), sigma)
    #
    # p_last = o[-3:].reshape(3, 1)
    # p_upadte = np.maximum(p_last - a.reshape(3,1),0)
    # sinr = (1 / (np.dot(F, p_upadte) + v)) * p_upadte  # 2*1,m=1
    # f_func = np.log(1 + sinr)
    #
    # GP = np.dot(G,p_upadte)[:,0]
    # gamma = sinr[:,0]
    # r = w[:,0] * f_func[:,0]

    # update_o = np.r_[GP, gamma, r, a]
    gamma_last = o[-3:]
    eta = 0.1
    gamma_upadte = np.maximum(gamma_last + eta * a, 0)
    f_func = np.log(1 + gamma_upadte)
    r = w[:,0] * f_func
    update_o = np.r_[r, gamma_upadte]


    obj_updated = w.T.dot(np.log(1 + gamma_upadte))[0]
    obj_star = w.T.dot(np.log(1 + gamma_star))[0]
    reward = - (np.abs(obj_updated-obj_star))**1
    # print("obj_updated:", obj_updated, "====", "obj_star:", obj_star)
    # reward = - (np.abs(obj_updated))
    # reward = - np.linalg.norm(gamma_upadte - gamma_star, 2)

    o2 = np.append(o[:-6], update_o)
    d = False
    if reward >= -1e-4:
        d = True
    if reward <= -1e6:
        print("!!!! large reward:", reward)
    return o2, reward, d
...
def main():
    ...
    while frame_idx < max_frames:
        ...
        for step in range(max_steps):
            ...
            next_state, reward, done = step_base(state, action, label)
            # print("action:", len(action))
            ddpg.replay_buffer.push(state, action, reward, next_state, done)

            if frame_idx >= start_update and len(ddpg.replay_buffer) > batch_size:
                ddpg.ddpg_update()
