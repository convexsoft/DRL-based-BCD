...
from _rl_sr_v2.td3_sr_v2 import TD3
import gym
...
if __name__ == '__main__':
    ...
    with open('res_reward_v3.csv', 'w', newline='') as file:
        for i in range(MAX_EPISODE):
            mywriter = csv.writer(file, delimiter=',')
            a = np.array(arr_rewardList[:,i])
            mywriter.writerow(a)
    with open('res_err_v3.csv', 'w', newline='') as file:
        for i in range(MAX_EPISODE):
            mywriter = csv.writer(file, delimiter=',')
            a = np.array(arr_err_list[:, i])
            mywriter.writerow(a)
    with open('res_step_v3.csv', 'w', newline='') as file:
        for i in range(MAX_EPISODE):
            mywriter = csv.writer(file, delimiter=',')
