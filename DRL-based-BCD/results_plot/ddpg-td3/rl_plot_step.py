...
my_list = list()
df=pd.read_csv('step_td3.csv', header=None, sep=',')
...
ddpg_list = list()
df=pd.read_csv('step_ddpg.csv', header=None, sep=',')
for i in range(start,len(df)-end):
...
ppo_list = list()
df=pd.read_csv('step_30.csv', header=None, sep=',')
...
plt.ylabel("Iteration steps", fontsize=fontsize)
